<template>
  <div class="spread-catalog" :class="{ 'spread-catalog--with-sidebar': showCategoryFilterComputed && !isMobile }">
    ...
    <!-- Postcode gate (replaces old region gate) -->
    <div v-if="regionRequired && !regionIdValue" class="spread-catalog__region-gate">
      <div class="spread-catalog__region-gate-card">
        ...
        <h3 class="spread-catalog__region-gate-title">Where do you want delivery?</h3>
        <p class="spread-catalog__region-gate-text">Enter your postcode to see local produce available near you.</p>
        <form class="spread-catalog__postcode-form" @submit.prevent="resolvePostcode">
          <input
            v-model="postcodeInput"
            class="spread-catalog__postcode-input"
            type="text"
            inputmode="numeric"
            maxlength="4"
            placeholder="e.g. 2000"
            :disabled="postcodeResolving"
          />
          <button
        ...
        <p v-if="postcodeError" class="spread-catalog__postcode-error">{{ postcodeError }}</p>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const _frontWin = typeof wwLib !== 'undefined' ? wwLib.getFrontWindow() : (typeof window !== 'undefined' ? window : null);
    const windowWidth = ref(_frontWin ? _frontWin.innerWidth : 1024);
    ...
    const resolvedRegionId = ref('');
    const postcodeInput = ref('');
    const postcodeResolving = ref(false);
    const postcodeError = ref('');
    const regionIdValue = computed(() => resolvedRegionId.value || props.content?.regionId || '');
    ...
    /* ---- Resolve postcode → region ---- */
    async function resolvePostcode() {
      const pc = postcodeInput.value.trim();
      if (!pc || pc.length < 3) return;

      postcodeResolving.value = true;
      postcodeError.value = '';

      try {
        const client = makeClient();
        const result = await client.rpc('resolve_service_area_for_client', { p_postcode: pc });
        const data = Array.isArray(result) ? result[0] : result;

        if (!data?.is_serviceable) {
          postcodeError.value = 'We don\'t deliver to that postcode yet. Try another one.';
          return;
        }

        resolvedRegionId.value = data.region_id;
        emit('trigger-event', {
          name: 'catalog:regionResolved',
          event: {
            postcode: pc,
        ...
        // Products will auto-load via the watcher on regionIdValue
      } catch (err) {
        postcodeError.value = err.message || 'Could not check that postcode. Please try again.';
      } finally {
        postcodeResolving.value = false;
      }
    }

    /* ---- Auto-resolve region for logged-in members ---- */
    async function autoResolveFromSavedAddress() {
      if (resolvedRegionId.value) return;
      if (props.content?.regionId) return; // externally provided — skip

      // Use deliveryPostcode prop (bound from member's saved address in WeWeb)
      const savedPostcode = props.content?.deliveryPostcode;
      ...
      try {
        const client = makeClient();
        const saResult = await client.rpc('resolve_service_area_for_client', { p_postcode: savedPostcode });
        const sa = Array.isArray(saResult) ? saResult[0] : saResult;
        if (sa?.is_serviceable) {
          resolvedRegionId.value = sa.region_id;
          emit('trigger-event', {
            name: 'catalog:regionResolved',
            event: {
              postcode: savedPostcode,
              regionId: sa.region_id,
              regionName: sa.region_name,
              stateName: sa.state_name,
            },
          });
        }
      } catch (_) {
        // Non-fatal — member will see the postcode gate instead
      }
    }
    ...
    /* ---- Responsive listener ---- */
    function onResize() {
      const w = typeof wwLib !== 'undefined' ? wwLib.getFrontWindow() : window;
      windowWidth.value = w.innerWidth;
    }

    /* ---- Lifecycle ---- */
    onMounted(async () => {
      fetchCategoryTree();
      const w = typeof wwLib !== 'undefined' ? wwLib.getFrontWindow() : (typeof window !== 'undefined' ? window : null);
      if (w) w.addEventListener('resize', onResize);
      // Auto-resolve region for logged-in members with saved address
      await autoResolveFromSavedAddress();
      fetchProducts(true);
    });

    onBeforeUnmount(() => {
      const w = typeof wwLib !== 'undefined' ? wwLib.getFrontWindow() : (typeof window !== 'undefined' ? window : null);
      if (w) w.removeEventListener('resize', onResize);
    });

    // Re-fetch when accessToken, accessMode, regionId, or resolved region changes
    watch(
      () => [props.content?.accessToken, props.content?.accessMode, props.content?.regionId, resolvedRegionId.value],
      () => {
        fetchProducts(true);
      },
    );

    // Auto-resolve if deliveryPostcode prop changes (e.g. member logs in, address loaded)
    watch(
      () => props.content?.deliveryPostcode,
      (newVal) => {
        if (newVal && !resolvedRegionId.value) {
          postcodeInput.value = newVal;
          autoResolveFromSavedAddress();
        }
      },
    );
...
.spread-catalog {
  ...
  font-family: 'Work Sans', ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;
  width: 100%;
  max-width: 1280px;
  margin: 0 auto;
  padding: 0 24px;
...
.spread-catalog__search-input::placeholder {
  color: var(--spread-text-disabled);
}
...
.spread-catalog__postcode-input {
  width: 120px;
  padding: 10px 14px;
  border: 1px solid var(--spread-border);
  border-radius: 8px;
...
.spread-catalog__postcode-input:focus {
  outline: none;
  border-color: var(--spread-accent);
}

.spread-catalog__postcode-btn {
  padding: 10px 20px;
  border: none;
  border-radius: 8px;
  background: var(--spread-accent);
...
.spread-catalog__postcode-btn:hover:not(:disabled) {
  background: var(--spread-accent-hover);
}

.spread-catalog__postcode-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.spread-catalog__postcode-error {
  margin-top: 10px;
  font-size: 13px;
  color: #dc2626;
  text-align: center;
...
.spread-catalog__card-oos-label {
  background: var(--spread-text-secondary);
  color: var(--white);
  padding: 6px 14px;
  border-radius: 20px;
  font-size: 12px;
...
.spread-catalog__skeleton-image {
  width: 100%;
  padding-top: 75%;
  background: linear-gradient(90deg, var(--spread-background) 25%, var(--spread-border-outer) 50%, var(--spread-background) 75%);
